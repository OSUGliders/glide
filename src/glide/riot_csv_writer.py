...
def write_riot_csv(
    ...
    rows_to_keep = np.logical_not(np.all(
        np.logical_or(
            np.isnan(temp_riot_array),
            temp_riot_array == 0),
        axis=0
    ))
    ...
    riot_df = pd.DataFrame({
        'riotData_prefix': np.full(
            len(epoch_msecs), '$riotData'),
    ...
    if add_positions:
        ...
        if np.all(abs(riot_ts - glider_ts) < TS_DIFF_THRESHOLD):
            ...
            riot_df['depth'] = np.interp(
                riot_ts, glider_ts, position_ds['depth'])
            riot_df['lat'] = np.interp(
                riot_ts, glider_ts, position_ds['lat'])
            riot_df['lon'] = np.interp(
                riot_ts, glider_ts, position_ds['lon'])
        else:
            _log.warning(
